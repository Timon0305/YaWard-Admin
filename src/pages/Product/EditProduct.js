...
class EditProduct extends Component {
    ...
    componentDidMount() {
        ...
        if (editData === undefined) {
            ...
            this.setState({
                'status': editData['status']['props']['children'],
            });
            ...
            axios.get(url + 'getVendor', {
                headers: {
                    'Authorization': token
                }
            }).then(res => {
                this.setState({vendor: res['data']['vendors']})
            });
            this.setState({
                'categoryName': editData['categoryName'],
                'occasionName': editData['occasionName'],
                'vendorName': editData['vendor_id']
            });
        }
    }
    ...
    render() {
        ...
        const vendors = this.state.vendor;
        const itemVendor = [];
        for (let i = 0; i < vendors.length; i++) {
            const vendorId = vendors[i]['_id'];
            const vendorName = vendors[i]['username'];
            itemVendor.push(<option key={i} value={vendorName}>{vendorName}</option>)
        }
            ...
            <React.Fragment>
                <div className="page-content">
                    <Container fluid>
                        ...
                        <Breadcrumbs title="Products" breadcrumbItem="Edit Products" />
                                ...
                                <Card>
                                    <CardBody>
                                        ...
                                        <AvForm onValidSubmit={this.handleSubmit}>
                                            ...
                                            <FormGroup className="mb-4" row>
                                                <label htmlFor="image" className="col-form-label col-lg-2">Select Vendors</label>
                                                <Col lg="10">
                                                    <AvField type="select" name='vendorName' value={this.state.vendorName}
                                                             onChange={this.handleInputChange}>
                                            ...
                                            <FormGroup className="mb-4" row>
                                                <label htmlFor="image" className="col-form-label col-lg-2">Status</label>
                                                <Col lg="10">
